...
import com.yeling.yelingziblog.user.service.MenuService;
...
import com.yeling.yelingziblog.common.utils.VerCodeGenerateUtil;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public LoginResp captchaLogin(EmailLoginReq req, String ip){
        // 检查登录尝试次数
        if (!allowLoginAttempt(ip)) {
            throw new TooManyAttemptsException();
        }
        // 校验验证码
        validateCaptcha(req.getEmail(), req.getVerifyCode());

        // 获取用户信息
        User user =  userMapper.getByUsername(req.getEmail());
