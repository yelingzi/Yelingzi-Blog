package com.yeling.yelingziblog.chat.service.Impl;

import com.github.houbb.sensitive.word.core.SensitiveWordHelper;
import com.yeling.yelingziblog.chat.dto.*;
import com.yeling.yelingziblog.chat.entity.Chat;
import com.yeling.yelingziblog.chat.entity.GroupChat;
import com.yeling.yelingziblog.chat.mapper.ChatMapper;
...
public class ChatServiceImpl implements ChatService {
    ...
    private static final String MessageTypeEmoji = "emoji";
    ...
    private static final Map<String, Set<String>> EMOJI_GROUPS = new HashMap<>();

    static {
        EMOJI_GROUPS.put("emoji", new HashSet<>(
                Arrays.asList("【微笑1】", "【微笑2】", "【大笑】","【哭】", "【睁眼】", "【惊吓】",
                        "【惊喜】", "【害羞】", "【吃奶】", "【Ciallo】", "【星声】", "【鸣潮】",
                        "【闭嘴】", "【点子】", "【棒棒糖】", "【墨镜】", "【杂鱼】", "【担心】")));
    ...
    public void sendMessage(User user, SendMessageReq sendMessageReq, String ip) {
        sendMessageInternal(user, sendMessageReq, ip, MessageTypeText);
    }

    @Override
    public void sendEmojiMessage(User user, SendMessageReq sendMessageReq, String ip) {
        validateEmoji(sendMessageReq.getMessage());
        sendMessageInternal(user, sendMessageReq, ip, MessageTypeEmoji);
    }

    private void sendMessageInternal(User user, SendMessageReq sendMessageReq, String ip, String messageType) {
        String userType = user.getId() == 0 ? "游客" : "用户";
        String action = sendMessageReq.getType().equals(SINGLE) ? "单聊" : "群聊";

        log.info("{}：{}，发送{} {} 消息：{}",
                userType, user.getNickname(), action, messageType, sendMessageReq.getMessage());

        if (sendMessageReq.getType().equals(SINGLE)) {
            sendSingleMessage(user, sendMessageReq.getMessage(), ip, messageType);
        } else if (sendMessageReq.getType().equals(GROUP)) {
            sendGroupMessage(user, sendMessageReq.getMessage(), ip, messageType);
        }
    }

    private void validateEmoji(String emoji) {
        boolean isValid = EMOJI_GROUPS.values().stream()
                .anyMatch(set -> set.contains(emoji));

        if (!isValid) {
            throw new BaseException(400, "请选择正确表情的发送");
        }
    }
    ...
    private void sendSingleMessage(User user, String message, String ip, String messageType) {
        ...
        String m = messageType.equals(MessageTypeText) ? checkMessage(message) : message;

        SingleChatMessageDto chatMessageDto = new SingleChatMessageDto(0, user.getId(), user.getNickname(), m, Admin_ID,  Admin_Nickname, ip, messageType);
    ...
    private void sendGroupMessage(User user, String message, String ip, String messageType) {
        // 检查消息是否合法
        String error = checkGroupMessage(message);
        if (error != null && Objects.equals(messageType, MessageTypeText)) {
            throw new BaseException(400, error);
        }

        GroupChatMessageDto chatMessageDto = new GroupChatMessageDto(0, user.getId(), user.getNickname(), user.getUserAvatar(), message, "chatroom", ip, messageType);
