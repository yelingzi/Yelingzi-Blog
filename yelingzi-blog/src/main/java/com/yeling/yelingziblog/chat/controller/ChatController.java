package com.yeling.yelingziblog.chat.controller;
...
public class ChatController {
    ...
    @PostMapping("/api/chat/send/emoji")
    public ApiResponse sendEmojiMessage(@RequestBody SendMessageReq sendMessageReq, HttpServletRequest request,
                                         @RequestHeader(value = "Authorization", required = false) String jwtToken,
                                         @RequestHeader("x-host") String deviceId) {

        User user = JwtUtils.getUserByTokenOrDeviceId(jwtToken, deviceId);

        chatService.sendEmojiMessage(user, sendMessageReq, IpUtils.getIpAddr(request));
        return ApiResponse.success();
    }
