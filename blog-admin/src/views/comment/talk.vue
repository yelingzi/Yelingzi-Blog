...
const commentList = ref<TalkComment[]>([])
...
const getMessageList = async () => {
    loading.value = true
    clearMessage()
    const res = await getTalkCommentListByPageService(page.value, pageSize.value)
...
const onDelMessage = async (id: number, index: any) => {
    await delTalkCommentService(id);
    ElMessage.success("删除成功")
    getMessageList()
}
const onPassMessage = async (id: number, index: any) => {
    await updateTalkCommentService(id);
