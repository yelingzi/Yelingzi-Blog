...
const messageList: MessageVO[] = reactive([])
...
const getMessageList = async () => {
    loading.value = true
    clearMessage()
    const res = await getMessageListByPageService(page.value, pageSize.value)
...
const onDelMessage = async (id: number, index: any) => {
    await delMessageService(id);
    ElMessage.success("删除成功")
    getMessageList()
}
const onPassMessage = async (id: number, index: any) => {
    await updateMessageService(id);
