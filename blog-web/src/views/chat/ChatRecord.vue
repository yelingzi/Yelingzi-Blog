<template>
  <div class="chat-wrapper" ref="chatWrapperRef">
    ...
    <div v-if="displayList.length === 0" class="no-message">向 TA 发送你的第一条消息吧 </div>
    <div v-else>
...
import type { Chat, ChatMessage } from '@/types/chatType'
...
watch(
  () => props.chating.chatMessageList.length,
  async () => {
    await nextTick()
    if (!chatWrapperRef.value) return
